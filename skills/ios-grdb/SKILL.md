...
**Note:** SQLiteData now supports GROUP BY (`.group(by:)`) and HAVING (`.having()`) via the query builder — see the `ios-sqlitedata-ref` skill.
...
**For migrations** See the `ios-database-migration` skill for safe schema evolution patterns.
...
**For migration safety patterns** See the `ios-database-migration` skill.
...
**Skills**: ios-database-migration, ios-sqlitedata, ios-swiftdata
