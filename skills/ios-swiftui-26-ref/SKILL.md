...
See `ios-swiftui-search-ref` for foundational `.searchable` APIs. iOS 26 adds:
...
- **Layout**: Use `.safeAreaPadding()` for edge-to-edge (not `.padding()`). See `ios-swiftui-layout-ref` for full guide
...
**Skills**: ios-swiftui-performance, ios-liquid-glass, ios-swift-concurrency, ios-app-intents-ref, ios-swiftui-search-ref
