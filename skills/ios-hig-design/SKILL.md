...
- `ios-hig` - Use for quick HIG decisions and checklists during day-to-day implementation.
- `ios-hig-ref` - Use for detailed API-level and guideline-level reference lookups.
- `ios-liquid-glass` - Use for iOS 26 material system decisions and implementation patterns.
- `ios-sf-symbols` - Use for symbol rendering/effects decisions that affect native polish.
- `ios-typography-ref` - Use for type scale, Dynamic Type, and legibility decisions.
- `design-system-doc` - Use to persist durable design principles in repo DESIGN.md.
