...
| 26 | Bottom-aligned search, `.searchToolbarBehavior(.minimize)`, `Tab(role: .search)`, `DefaultToolbarItem(kind: .search)` — see `ios-swiftui-26-ref` |
...
For iOS 26 search features (bottom-aligned, minimized toolbar, search tab role), see `ios-swiftui-26-ref`.
...
For bottom-aligned search, `.searchToolbarBehavior(.minimize)`, `Tab(role: .search)`, and `DefaultToolbarItem(kind: .search)`, see `ios-swiftui-26-ref`. These build on the foundational APIs documented here.
...
**Skills**: ios-swiftui-26-ref, ios-swiftui-nav-ref, ios-swiftui-nav
