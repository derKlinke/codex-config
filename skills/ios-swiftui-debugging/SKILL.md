...
**Related skills**: `ios-xcode-debugging` (cache corruption diagnosis), `ios-swift-concurrency` (observer patterns), `ios-swiftui-performance` (profiling with Instruments), `ios-swiftui-layout` (adaptive layout patterns)
...
**Cross-reference**: For complex update patterns, use SwiftUI Instrument → see `ios-swiftui-performance` skill
...
**Skills**: ios-swiftui-performance, ios-swiftui-debugging-diag, ios-xcode-debugging, ios-swift-concurrency, ios-lldb (LLDB debugging workflows beyond Self._printChanges)
