...
| App feels slow but responsive | No — use ios-performance-profiling |
| Memory grows over time | No — use ios-memory-debugging first |
| App completely frozen | Maybe — use ios-hang-diagnostics first, then LLDB for thread inspection |
| Crash in production, no local repro | No — use ios-testflight-triage first |
...
digraph tool_selection {
    ...
    "ios-testflight-triage" [shape=box];
    "ios-hang-diagnostics" [shape=box];
    "ios-memory-debugging" [shape=box];
    "ios-performance-profiling" [shape=box];
    "LLDB (this skill)" [shape=box, style=bold];

    "What do you need?" -> "ios-testflight-triage" [label="Crash log from field,\ncan't reproduce locally"];
    "What do you need?" -> "ios-hang-diagnostics" [label="App frozen,\nneed diagnosis approach"];
    "What do you need?" -> "ios-memory-debugging" [label="Memory growing,\nneed leak pattern"];
    "What do you need?" -> "ios-performance-profiling" [label="Need to measure\nCPU/memory over time"];
...
**Cross-reference:** For fix patterns once you've identified the hang cause → `/skill ios-hang-diagnostics`
...
For the full observation diagnostic tree → `/skill ios-swiftui-debugging`
...
**Cross-reference:** For Swift concurrency patterns and fix strategies → `/skill ios-swift-concurrency`. For profiling async performance → `/skill ios-concurrency-profiling`
...
6. Last resort — read register values directly (variables live in registers before being optimized out):
   ...
   ```
   On ARM64: `x0` = self, `x1`-`x7` = first 7 arguments. Check `/skill ios-lldb-ref` Part 1 for details.
...
**Skills**: ios-lldb-ref, ios-testflight-triage, ios-hang-diagnostics, ios-memory-debugging, ios-swift-concurrency, ios-concurrency-profiling
