...
- Integrating with simulator screenshot workflows
...
### With `simctl` Screenshot Command
...
xcrun simctl io booted screenshot /tmp/deeplink-debug-$(date +%s).png
```

### With Manual Simulator Flow

Use debug deep links plus `simctl` screenshots for repeatable visual checks.
...
- [ ] Works with `xcrun simctl io booted screenshot /tmp/deeplink-debug-$(date +%s).png` command
- [ ] Works with your simulator automation workflow
