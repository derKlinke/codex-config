...
**Related references**: `references/deep/debugging-diag.md` (systematic diagnostics), `references/deep/performance.md` (profiling with Instruments), `references/deep/layout.md` (adaptive layout patterns), `references/swift.md` (observer/concurrency patterns)
...
→ This reference identifies accidental view recreation from conditionals and shows `.opacity()` fix

#### 4. "I'm using @Observable but the view still doesn't update when I change the property"
→ This reference explains when to use @State vs plain properties with @Observable objects

#### 5. "Text field loses focus when I start typing, very frustrating"
→ This reference identifies ForEach identity issues and shows how to use stable IDs
...
#### Use `ios-xcode-debugging` instead when
- App crashes at runtime (not preview)
- Build fails completely
- You need environment diagnostics

#### Use `ios-swift-concurrency` instead when
...
**Cross-reference**: For complex update patterns, use SwiftUI Instrument → see `references/deep/performance.md`
...
> "I appreciate the suggestion. Adding @ObservedObject everywhere is treating the symptom, not the root cause. The diagnostic protocol says intermittent bugs create NEW bugs when we guess. I need 60 minutes for systematic diagnosis. If I can't find the root cause by then, we'll disable the feature and ship a clean v1.1. The math shows we have time—I can complete diagnosis, fix, AND verification before the deadline."
...
**Diagnosis using this reference**:
...
xcrun simctl io booted screenshot /tmp/ui-debug-$(date +%s).png
...
```bash
# 1. Add debug deep links (see `references/deep/navigation-diag.md`)
...
```bash
Run your simulator test workflow for multi-step validation
```
...
**Related References**: `references/deep/debugging-diag.md`, `references/deep/performance.md`, `references/deep/layout.md`, `references/swift.md`


## Local Cross-References

- `ios-hig-design` - Use when a bug fix affects interaction clarity, hierarchy, or native iOS behavior.
- `design-system-doc` - Use when repeated UI fixes indicate durable design rules that should be captured in DESIGN.md.
- `ios-debugger-agent` - Use for simulator-driven reproduction, UI inspection, and verification screenshots/logs.
