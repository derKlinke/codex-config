...
2. Scan relevant existing notes before drafting.
3. Read `references/klinke-studio-rules.md` when working in Fabian's `klinke.studio` vault.
4. Create or edit note content with atomic concept scope and inline wikilink integration.
5. Apply citation, visual, and language constraints from the reference, including visual tooling defaults (`matplotlib` and PlantUML).
6. Run final quality check: structure, links, formulas, citations, transparency line.
...
- Language/transparency policy: keep notes monolingual (preserve existing note language); no `see also` blocks; include one emphasized footer line in note language (`_mitverfasst von einem KI-Agenten._` or `_co-authored by an AI agent._`).
...
- Add the transparency line at note end in the note language.
