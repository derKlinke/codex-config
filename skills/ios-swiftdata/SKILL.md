...
**For migrations** See the `ios-swiftdata-migration` skill for custom schema migrations with VersionedSchema and SchemaMigrationPlan. For migration debugging, see `ios-swiftdata-migration-diag`.
...
→ See the comparison table in Migration section below, then follow `realm-to-swiftdata-migration` or `ios-swiftdata-migration` for detailed guides
...
SwiftData CloudKit sync uses **last-write-wins** by default. For sync status monitoring, custom conflict resolution, and offline-aware UI patterns, see `ios-cloud-sync`. For CKShare-based record sharing, see `ios-cloudkit-ref`.
...
- **`ios-swiftdata-migration`** — SwiftData schema evolution: VersionedSchema, SchemaMigrationPlan, lightweight vs custom migrations
- **`ios-database-migration`** — Safe additive migration patterns applicable to any persistence framework
...
**Skills**: ios-swiftdata-migration, ios-swiftdata-migration-diag, ios-database-migration, ios-sqlitedata, ios-grdb, ios-swift-concurrency
