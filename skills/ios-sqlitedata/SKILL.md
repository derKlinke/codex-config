...
For complex joins across 4+ tables, drop down to raw GRDB (see `ios-grdb`).
...
**For schema creation** (CREATE TABLE, migrations), see the `ios-sqlitedata-ref` reference skill for complete examples.
...
**Skills**: ios-sqlitedata-ref, ios-sqlitedata-migration, ios-database-migration, ios-grdb
