...
See `ios-accessibility-diag` for systematic auditing before declaring labels.
...
- `ios-privacy-ux` — Deep implementation of privacy manifests, ATT, Required Reason APIs
- `ios-storekit-ref` — IAP and subscription implementation
- `ios-accessibility-diag` — Accessibility audit before declaring Nutrition Labels
- `ios-testflight-triage` — TestFlight crash and feedback triage
- `ios-app-store-connect-ref` — ASC navigation, crash data, metrics
- `ios-xcode-debugging` — Build failures and environment issues
...
**Skills**: ios-privacy-ux, ios-storekit-ref, ios-accessibility-diag, ios-testflight-triage
...
- `ios-hig-design` - Use when resolving review feedback tied to native interaction, accessibility, and platform conventions.
- `design-system-doc` - Use to keep documented design policy aligned with what is submitted and reviewed.
- `security-review` - Use to catch security/privacy defects that commonly block or delay review outcomes.
