...
- `swiftui-liquid-glass` — Design principles, implementation patterns, and adoption guidance
...
See `swiftui-nav` for `Tab(role: .search)` patterns and Tab customization guidance.
...
**Cross-reference** `swift-concurrency-expert` — Swift 6 strict concurrency patterns
...
**Cross-reference** `swiftui-animation-ref` — Comprehensive animation guide covering VectorArithmetic, Animatable protocol, @Animatable macro, animation types, Transaction system, and performance optimization
...
**Cross-reference** Apple App Intents documentation — AttributedString for Apple Intelligence Use Model action
...
**Reference**: See `swiftui-layout` for `.safeAreaPadding()` vs `.padding()` guidance, or `swiftui-liquid-glass` for Liquid Glass-specific safe area patterns.
...
**Skills**: swiftui-performance, swiftui-liquid-glass, swift-concurrency-expert
