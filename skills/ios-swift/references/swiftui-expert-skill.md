...
Before styling or component-level UI work:

1. Check `<repo-root>/DESIGN.md`.
2. Treat it as source of truth for tokens, typography scale, component variants, and motion constraints.
3. If missing and task includes UI/design work, create it via `design-system-doc` template.
4. Update `DESIGN.md` in the same task when SwiftUI component styling/tokens change.

## Workflow Decision Tree

### 1) Review existing SwiftUI code
- Check property wrapper usage against the selection guide (see `swiftui-expert-skill--state-management.md`)
- Verify modern API usage (see `swiftui-expert-skill--modern-apis.md`)
- Verify view composition follows extraction rules (see `swiftui-expert-skill--view-structure.md`)
- Check performance patterns are applied (see `swiftui-expert-skill--performance-patterns.md`)
- Verify list patterns use stable identity (see `swiftui-expert-skill--list-patterns.md`)
- Check animation patterns for correctness (see `swiftui-expert-skill--animation-basics.md`, `swiftui-expert-skill--animation-transitions.md`)
- Inspect Liquid Glass usage for correctness and consistency (see `swiftui-expert-skill--liquid-glass.md`)
- Validate iOS 26+ availability handling with sensible fallbacks

### 2) Improve existing SwiftUI code
- Audit state management for correct wrapper selection (prefer `@Observable` over `ObservableObject`)
- Replace deprecated APIs with modern equivalents (see `swiftui-expert-skill--modern-apis.md`)
- Extract complex views into separate subviews (see `swiftui-expert-skill--view-structure.md`)
- Refactor hot paths to minimize redundant state updates (see `swiftui-expert-skill--performance-patterns.md`)
- Ensure ForEach uses stable identity (see `swiftui-expert-skill--list-patterns.md`)
- Improve animation patterns (use value parameter, proper transitions, see `swiftui-expert-skill--animation-basics.md`, `swiftui-expert-skill--animation-transitions.md`)
- Suggest image downsampling when `UIImage(data:)` is used (as optional optimization, see `swiftui-expert-skill--image-optimization.md`)
- Adopt Liquid Glass only when explicitly requested by the user

### 3) Implement new SwiftUI feature
- Design data flow first: identify owned vs injected state (see `swiftui-expert-skill--state-management.md`)
- Use modern APIs (no deprecated modifiers or patterns, see `swiftui-expert-skill--modern-apis.md`)
- Use `@Observable` for shared state (with `@MainActor` if not using default actor isolation)
- Structure views for optimal diffing (extract subviews early, keep views small, see `swiftui-expert-skill--view-structure.md`)
- Separate business logic into testable models (see `swiftui-expert-skill--layout-best-practices.md`)
- Use correct animation patterns (implicit vs explicit, transitions, see `swiftui-expert-skill--animation-basics.md`, `swiftui-expert-skill--animation-transitions.md`, `swiftui-expert-skill--animation-advanced.md`)
- Apply glass effects after layout/appearance modifiers (see `swiftui-expert-skill--liquid-glass.md`)
...
### Modern APIs (see `swiftui-expert-skill--modern-apis.md`)
...
### Sheets & Navigation (see `swiftui-expert-skill--sheet-navigation-patterns.md`)
- [ ] Using `.sheet(item:)` for model-based sheets
- [ ] Sheets own their actions and dismiss internally
- [ ] Using `navigationDestination(for:)` for type-safe navigation

### ScrollView (see `swiftui-expert-skill--scroll-patterns.md`)
...
### Text & Formatting (see `swiftui-expert-skill--text-formatting.md`)
- [ ] Using modern Text formatting (not `String(format:)`)
- [ ] Using `localizedStandardContains()` for search filtering

### View Structure (see `swiftui-expert-skill--view-structure.md`)
- [ ] Using modifiers instead of conditionals for state changes
- [ ] Complex views extracted to separate subviews
- [ ] Views kept small for performance
- [ ] Container views use `@ViewBuilder let content: Content`

### Performance (see `swiftui-expert-skill--performance-patterns.md`)
...
### List Patterns (see `swiftui-expert-skill--list-patterns.md`)
- [ ] ForEach uses stable identity (not `.indices`)
- [ ] Constant number of views per ForEach element
- [ ] No inline filtering in ForEach
- [ ] No `AnyView` in list rows

### Layout (see `swiftui-expert-skill--layout-best-practices.md`)
...
### Animations (see `swiftui-expert-skill--animation-basics.md`, `swiftui-expert-skill--animation-transitions.md`, `swiftui-expert-skill--animation-advanced.md`)
...
## References
- `swiftui-expert-skill--state-management.md` - Property wrappers and data flow (prefer `@Observable`)
- `swiftui-expert-skill--view-structure.md` - View composition, extraction, and container patterns
- `swiftui-expert-skill--performance-patterns.md` - Performance optimization techniques and anti-patterns
- `swiftui-expert-skill--list-patterns.md` - ForEach identity, stability, and list best practices
- `swiftui-expert-skill--layout-best-practices.md` - Layout patterns, context-agnostic views, and testability
- `swiftui-expert-skill--modern-apis.md` - Modern API usage and deprecated replacements
- `swiftui-expert-skill--animation-basics.md` - Core animation concepts, implicit/explicit animations, timing, performance
- `swiftui-expert-skill--animation-transitions.md` - Transitions, custom transitions, Animatable protocol
- `swiftui-expert-skill--animation-advanced.md` - Transactions, phase/keyframe animations (iOS 17+), completion handlers (iOS 17+)
- `swiftui-expert-skill--sheet-navigation-patterns.md` - Sheet presentation and navigation patterns
- `swiftui-expert-skill--scroll-patterns.md` - ScrollView patterns and programmatic scrolling
- `swiftui-expert-skill--text-formatting.md` - Modern text formatting and string operations
- `swiftui-expert-skill--image-optimization.md` - AsyncImage, image downsampling, and optimization
- `swiftui-expert-skill--liquid-glass.md` - iOS 26+ Liquid Glass API
