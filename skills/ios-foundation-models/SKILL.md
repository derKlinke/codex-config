...
description: Use when implementing on-device AI with Apple's Foundation Models framework — prevents context overflow, blocking UI, wrong model use cases, and manual JSON parsing when @Generable should be used. iOS 26+, macOS 26+, iPadOS 26+, ios-visionOS 26+
license: MIT
...
#### Related Skills
- Use `ios-foundation-models-diag` for systematic troubleshooting (context exceeded, guardrail violations, availability problems)
- Use `ios-foundation-models-ref` for complete API reference with all WWDC code examples
...
Supports `String`, `Int`, `Float`, `Double`, `Bool`, arrays, nested `@Generable` types, enums with associated values, and recursive types. See `ios-foundation-models-ref` for complete list with examples.
...
See `ios-foundation-models-ref` for complete `@Guide` reference (ranges, regex, maximum counts).
...
`@Generable` macro automatically creates a `PartiallyGenerated` type where all properties are optional (they fill in as the model generates them). See `ios-foundation-models-ref` for details.
...
See `ios-foundation-models-ref` for `Tool` protocol reference, `ToolOutput` forms, stateful tool patterns, and additional examples.
...
For advanced strategies (summarizing middle entries with Foundation Models itself), see `ios-foundation-models-ref`.
...
See `ios-foundation-models-ref` for complete `GenerationOptions` API reference.
...
See `ios-foundation-models-ref` for code examples of each optimization.
...
**Skills**: ios-foundation-models-diag, ios-foundation-models-ref

---

**Last Updated**: 2025-12-03
**Version**: 1.0.0
**Target**: iOS 26+, macOS 26+, iPadOS 26+, ios-visionOS 26+
