...
### Related Skills
- `ios-energy-ref` — Complete API reference with all code examples
- `ios-energy-diag` — Symptom-based troubleshooting decision trees
- `ios-background-processing` — Background task mechanics (why tasks don't run)
- `ios-performance-profiling` — General Instruments workflows
- `ios-memory-debugging` — Memory leak diagnosis (often related to energy)
- `ios-networking` — Network optimization patterns
