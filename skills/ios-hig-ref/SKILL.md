...
#### Related Skills
- Use `ios-hig` for quick decisions and checklists
- Use `ios-liquid-glass` for iOS 26 material implementation
- Use `ios-liquid-glass-ref` for iOS 26 app-wide adoption
...
**Cross-reference:** For full Liquid Glass implementation patterns (sheets, alerts, popovers, morphing transitions), see `ios-liquid-glass-ref`. For decision trees, see `ios-liquid-glass`.
...
> **For comprehensive coverage** of rendering modes (Monochrome, Hierarchical, Palette, Multicolor), symbol effects (Bounce, Pulse, Wiggle, Draw On/Off), and custom symbol authoring, see `ios-sf-symbols` (decision trees) and `ios-sf-symbols-ref` (complete API).
...
**Not required for:** macOS, ios-visionOS, watchOS
...
- `ios-hig-design` - Use for actionable review criteria when moving from reference lookup to implementation decisions.
- `design-system-doc` - Use to encode selected HIG-aligned rules as stable design-system constraints.
