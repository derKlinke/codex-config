...
Using Clear without meeting all three conditions results in poor legibility. See `ios-liquid-glass-ref` for implementation examples.
...
Scroll edge effects dissolve content into background as it scrolls, lifting glass above moving content. Use `.scrollEdgeEffect(.hard)` when pinned accessory views exist (e.g., column headers) for extra visual separation. See `ios-liquid-glass-ref` for full API details.
...
See `ios-ui-testing` for comprehensive UI testing patterns including visual regression and accessibility testing.
...
- UIKit views use the UIKit Liquid Glass APIs (see `ios-liquid-glass-ref` for migration mapping)
- Hosted SwiftUI views inside `UIHostingController` get glass effects independently

See `ios-liquid-glass-ref` for complete UIBlurEffect migration mapping table.
...
For complete API reference including `glassEffect()`, `glassBackgroundEffect()`, toolbar modifiers, scroll edge effects, navigation/search APIs, controls/layout, `GlassEffectContainer`, `glassEffectID`, types, and backward compatibility, see `ios-liquid-glass-ref`.
...
**Skills**: ios-liquid-glass-ref

---

**Platforms:** iOS 26+, iPadOS 26+, macOS Tahoe, ios-visionOS 3
**Xcode:** 26+
**History:** See git log for changes

## Local Cross-References

- `ios-hig-design` - Use to validate Liquid Glass choices against broader iOS interaction and accessibility principles.
- `design-system-doc` - Use to capture Liquid Glass adoption boundaries and token-level rules in DESIGN.md.
