---
name: ios-extensions-widgets-ref
...
license: MIT
compatibility: iOS 14+, iPadOS 14+, watchOS 9+, macOS 11+, ios-visionOS 2+
metadata:
  version: "1.0.0"
---
...
The intent updates shared data via App Groups in its `perform()` method. See **ios-app-intents-ref** for full `AppIntent` definition syntax.
...
The intent follows the same `AppIntent` structure with a `@Parameter(title: "Enabled") var enabled: Bool`. See **ios-app-intents-ref** for full `AppIntent` definition syntax.
...
**Version**: 0.9 | **Platforms**: iOS 14+, iPadOS 14+, watchOS 9+, macOS 11+, ios-visionOS 2+
