...
#### Related Skills
- Use `ios-networking-diag` for systematic troubleshooting of connection failures, timeouts, and performance issues
- Use `ios-network-framework-ref` for comprehensive API reference with all WWDC examples
...
// Blocks, no batching, ios-high CPU overhead
```
...
For apps supporting iOS 12-25 that can't use async/await yet, invoke `/skill ios-networking-legacy`:
...
For detailed migration guides from legacy networking APIs, invoke `/skill ios-networking-migration`:
...
**Skills**: ios-networking-diag, ios-network-framework-ref
